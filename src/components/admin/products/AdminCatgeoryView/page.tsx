...
export default function Page() {
  const { error, loading, categories, fetchCatgeory } = Catgeory();
  const deletePlatform = async (id: string) => {
    try {
      await axios.delete(`/api/product/catgeory?catgeoryId=${id}`);
    ...
    <div className="mx-auto md:px-6 px-4 my-8">
      ...
      >
        {categories.map((catgeory: any) => (
          ...
          >
            <Link
              href={`/dashboard/admin/products/put/${catgeory._id}`}
